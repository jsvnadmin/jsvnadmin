...
<table class="sortable thinborder">
	...
	if(list!=null){
	  for(int i = 0;i<list.size();i++){
		...
		<td>
		<a href="javascript:if(confirm('<%=I18N.getLbl(request,"pjauth.op.delete.confirm","确认删除?") %>')){del('<%=_ctx%>/pjauth?pj=<%=pjAuth.getPj()%>&res=<%=pjAuth.getRes()%>&gr=<%=pjAuth.getGr()%>&usr=<%=pjAuth.getUsr()%>&path=<%=request.getParameter("path")==null?"":request.getParameter("path")%>')}"><%=I18N.getLbl(request,"pjauth.op.delete","删除") %></a>
		</td>
	</tr>
		<%	
	}}
